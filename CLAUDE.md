...
- The **auth feature alone** follows Clean Architecture: `domain/` (entities, repository interface, use cases) → `data/` (repository impl) → `presentation/`. Use cases (`auth_usecases.dart`) cover login, register, logout, password recovery/reset, profile + password update, current user; their providers live at the top of `auth_viewmodel.dart` (not in `core/providers.dart`). `AuthState.inicializando` is `true` while the persisted session is being restored at startup. Session restore (`_restaurarSesion`) uses `PreferenciasAcceso` (shared_preferences keys `recordarme`, `biometria_habilitada`, `ultimo_email`, which survive logout): no Recordarme/biometría → the Supabase session is signed out; biometría on → session stays locked (`sesionBloqueada`) until `loginBiometrico()` passes the `BiometricService` (`local_auth`) prompt; Recordarme only → auto-login. No password is ever stored; biometrics only unlock the Supabase-persisted session, so it doesn't work after an explicit logout. Other features go straight **View → ViewModel → Repository → Supabase** with no domain layer.
...
- `initialLocation` is `/splash`. While `authState.inicializando`, everything redirects to `/splash`; once done, `/splash` goes to `/dashboard` only if "Recordarme" restored the session, otherwise `/login`.
